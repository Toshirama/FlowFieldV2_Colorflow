...
var inc = 0.1;                    //inc>0.4 wurzel strukturen
var incStart = 0.005;
var magInc = 0.0005;
var start = 0;
var scl = 25;
...
function draw() {
  if (showField) {
    ...
    background(color(0, 0, 0, 25));                            //laenge gezeichnete linie
  }
  var yoff = start;
  for (let y = 0; y < rows; y++) {
    ...
    for (let x = 0; x < cols; x++) {
      ...
      let m = map(noise(xoff, yoff, magOff), 0, 1, -1, 1);     // Anziehung Matrix
      v.setMag(m);
      if (showField) {
        push();
        stroke(0);
